...
create policy "admin or owner read gym owners" on public.gym_owners for select to authenticated using (is_admin((select auth.uid())) or user_id = (select auth.uid()));
