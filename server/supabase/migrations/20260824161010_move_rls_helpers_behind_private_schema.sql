...
as $$
  select case
    when _user_id is null or _user_id is distinct from (select auth.uid()) then false
    else private.has_role(_user_id, _role)
  end;
$$;
...
as $$
  select case
    when _user_id is null or _user_id is distinct from (select auth.uid()) then false
    else private.has_role(_user_id, 'admin'::public.app_role)
...
as $$
  select case
    when _user_id is null or _user_id is distinct from (select auth.uid()) then false
    else private.has_role(_user_id, 'super_admin'::public.app_role)
  end;
$$;
...
as $$
  select case
    when p_user_id is null or p_user_id is distinct from (select auth.uid()) then false
    else private.owns_gym(p_gym_id, p_user_id)
  end;
$$;
