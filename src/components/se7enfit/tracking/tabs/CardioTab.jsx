...
export default function CardioTab({ profile, refreshToken = 0 }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
        ...
        <div><Label className="text-[11px]">Activity</Label><Select value={form.activity} onValueChange={v => setForm(p => ({ ...p, activity: v }))}><SelectTrigger className="mt-0.5 h-10 rounded-xl bg-background border-border"><SelectValue /></SelectTrigger><SelectContent>{ACTIVITIES.map(a => <SelectItem key={a} value={a}>{ACTIVITY_EMOJI[a]} {a.charAt(0).toUpperCase() + a.slice(1)}</SelectItem>)}</SelectContent></Select></div>
