...
import { Activity, Flame, MapPin, Clock, Check } from 'lucide-react';
        ...
        {todayLogs.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-card border border-border rounded-xl p-3 text-center">
              ...
              <p className="text-sm font-bold">{todayMinutes}m</p>
        ...
        <div className="bg-card border border-border rounded-2xl p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="number" placeholder="30" value={form.duration_minutes} onChange={e => setForm(p => ({ ...p, duration_minutes: e.target.value }))} className="h-10 rounded-xl bg-background border-border" />
            </div>
            <div>
              <Label className="text-xs mb-1 block">Distance (km)</Label>
              <Input type="number" placeholder="5" value={form.distance_km} onChange={e => setForm(p => ({ ...p, distance_km: e.target.value }))} className="h-10 rounded-xl bg-background border-border" step="0.1" />
            </div>
            <div>
              <Label className="text-xs mb-1 block">Avg Heart Rate</Label>
              <Input type="number" placeholder="150" value={form.avg_heart_rate} onChange={e => setForm(p => ({ ...p, avg_heart_rate: e.target.value }))} className="h-10 rounded-xl bg-background border-border" />
            </div>
            <div>
              <Label className="text-xs mb-1 block">Notes</Label>
              <Input placeholder="Optional" value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} className="h-10 rounded-xl bg-background border-border" />
            </div>
          </div>
          <Button onClick={handleSave} disabled={saving || !form.duration_minutes} className="w-full h-10 rounded-xl bg-accent text-accent-foreground">
            <Check size={14} className="mr-1" /> Log Session
          </Button>
        </div>

        {logs.length > 0 && (
          <div>
            <h3 className="font-heading font-semibold text-sm mb-3">Recent Sessions</h3>
            <div className="space-y-2">
              {logs.slice(0, 10).map(log => {
                const act = ACTIVITIES.find(a => a.key === log.activity);
                return (
                  <div key={log.id} className="bg-card border border-border rounded-xl p-3 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium">{act?.label || log.activity}</p>
                      <p className="text-xs text-muted-foreground">{log.date} • {log.duration_minutes}min</p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-bold text-accent">{log.calories_burned} kcal</p>
